...
export default function KnowledgePage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        const updated = await knowledge.update(editing.id, form);
      ...
      if (tab === 'file') {
        ...
        const created = await knowledge.upload(file, form.title, form.category, form.is_active);
        setDocs((d) => [...d, created]);
      } else {
        const created = await knowledge.create(form);
        setDocs((d) => [...d, created]);
      }
      closeForm();
    } catch (err) {
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-900">{t.knowledge.title}</h1>
